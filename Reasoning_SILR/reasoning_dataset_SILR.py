...
class ReasoningChainDataset(Dataset):
    def __init__(self,
        ...
        self.type_descriptions = {
            ...
            '4-chain': "four steps: [projection], then [projection], then [projection], then [projection]", 
            '5-chain': "five steps: [projection], then [projection], then [projection], then [projection], then [projection]",
            '3-inter-chain': "seven steps: [projection], then [projection], and [projection], then [projection], and [projection], then [projection], [intersection]", 
            'inter-2-chain': "seven steps: [projection], then [projection], then [projection], and [projection], then [projection], then [projection], [intersection]"
        }
    ...
    def __getitem__(self, index):
        ...
        return_dict = {
            ...
            "entity_positions": torch.LongTensor(entity_positions),
            "relation_positions": torch.LongTensor(relation_positions),
...
class ReasoningInterDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for each in sample["query_text"]:
            query += "[projection] [anchor] " + each[0] + "".join([" [projection] [rela] " + _ if i > 0 else " [rela] " + _ for i, _ in enumerate(each[1])]) + " " #+ " [MASK]"
...
class ReasoningInterChainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        query = ""
        if query_type in ["inter-2-chain"]:
            for each in sample["query_text"][:-2]:
                query += "[projection] [anchor] " + each[0] + "".join([" [projection] [rela] " + _ if i > 0 else " [rela] " + _ for i, _ in enumerate(each[1])]) + " [projection] [rela] " + sample["query_text"][-2] + " [projection] [rela] " + sample["query_text"][-1] + " " 
            ...
            for each in sample["query_text"][:-1]:
                query += "[projection] [anchor] " + each[0] + "".join([" [projection] [rela] " + _ if i > 0 else " [rela] " + _ for i, _ in enumerate(each[1])]) + " [projection] [rela] " + sample["query_text"][-1] + " " 
                if query_type == "inter-chain":
                    query = "[intersection] " + query 
                else:
                    query = "[union] " + query 
        ...
        if query_type not in ['3-inter-chain', 'inter-2-chain']:
            assert anchor_count == 2
            assert rela_count == 4
            for i in range(anchor_count):
                cur_anchor_loc = input_ids.index(anchor_token_id, find_anchor_start)
            ...
            assert len(entity_positions) == 6 
            assert len(relation_positions) == 12
        else:
            assert anchor_count == 2
            assert rela_count == 6
            for i in range(anchor_count):
                ...
                cur_rela_loc_3 = input_ids.index(rela_token_id, find_rela_start)
                find_rela_start = cur_rela_loc_3 + 1

                relation_positions.append(cur_rela_loc_3 - 2)
                relation_positions.append(cur_rela_loc_3)
        ...
        for i in range(len(samples)):
            ...
            for j in range(negative_num+1):
                if samples[negative_index_list[i][j]]["selected_ans"] in samples[i]["ans"]:
                    cur_tag.append(torch.LongTensor([1]))
                else:
                    cur_tag.append(torch.LongTensor([0]))
            tag_list.append(cur_tag)
        
        label_tags = []
        for i in range(len(tag_list[0])):
            label_tags += [each_tag[i].view(-1) for each_tag in tag_list]
        batch["tags"] = collate_tokens(label_tags, -1)
